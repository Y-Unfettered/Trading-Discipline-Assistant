...
import { Archive, Bot, Copy, Eye, FilePlus2, Pencil, ShieldCheck, Target } from 'lucide-vue-next'
...
<template>
  <AssetDetailView v-if="selectedAsset" :asset="selectedAsset" :evidence="evidence" @back="selectedAssetCode = ''" @edit="editAsset" @add-evidence="startEvidence" />
  ...
  <PageHeading eyebrow="Asset center & evidence" title="交易标的中心" description="当前持仓自动出现；这里只添加尚未持有、但准备研究的候选标的。">
    ...
    <template #actions><Button variant="outline" @click="openCandidateForm()"><Target class="size-4" />候选标的</Button><Button variant="outline" @click="startEvidence()"><FilePlus2 class="size-4" />证据卡</Button><Button variant="outline" @click="aiDialogOpen = true"><Bot class="size-4" />AI 研究接入</Button></template>
  </PageHeading>
    ...
    <DialogScrollContent class="max-w-4xl">
      ...
      <form id="candidate-dialog-form" class="space-y-4" @submit.prevent="saveAsset">
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <FieldBlock label="行业"><Input v-model="assetForm.industry" placeholder="所属行业" /></FieldBlock>
        </div>
